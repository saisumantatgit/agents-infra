...
created: 2026-07-08
ack_required: true
acceptance_criteria: labeling-v2.csv rows corrected/ratified and label_status flipped to gold
---
...
- **File to edit:** `Agent-Assure/calibration/labeling-v2.csv`

Three steps: (1) for each row, confirm or correct `human_label`
(grounded/violation); (2) leave `candidate_verdict` and `rationale` as the
frozen record; (3) find-and-replace `label_status` `candidate` → `gold` on
every row, and save.
...
`scripts.calibrate.load_labels` refuses to ingest the file while any row is
`label_status=candidate` (fail-loud, tested in
`tests/test_labeling_v2_candidate_guard.py`). Calibration runs on gold labels
only — so α2 (threshold sweep, leave-one-out, CR-002) cannot start until you
flip the file to gold.
